...
var player = new sprite(basket_img, 200, 430, 115, 100);//, document.getElementById('game'));
...
window.addEventListener("keydown", (event) => {
    if(!play){return}
    //if(event.key == 'A' || event.key == 'a' || event.key == 'ArrowLeft'){if(isOverflown(game_screen)){scroll_to_left();}}
    //if(event.key == 'D' || event.key == 'd' || event.key == 'ArrowRight'){if(isOverflown(game_screen)){scroll_to_right();}}
    switch(event.key){
        case 'A':
        case 'a':
        case 'ArrowLeft':
...
var menu_interval = setInterval(function () {
    ...
    /*a2.style.width=(window.innerHeight/7)+'px';
    a2.style.height=(window.innerHeight/7)+'px';
    a2.style.display='block';
    a2.style.position='absolute';
    a2.style.left=(window.innerHeight/8)*8 + 'px';
    a2.style.top=(window.innerHeight/20)+'px';*/
...
var main_interval = setInterval(function(){
    ...
    if(global_height!=pasth || global_width!=pastw){
        pasth=global_height;
        pastw=global_width;
        game_resize();
    }
...
function get_random_apple_type(){
    ...
    for (let index = 0; index < apples.length; index++) {
        if(apples[index][1] > score || apples[index][1] == 0){
            options.push(apples[index]);//asa cred ca va fi sistemul si la enemies
        }
...
function get_random_enemy_type(){
    ...
    for (let index = 0; index < enemy.length; index++) {
        if(enemy[index][1] > score || enemy[index][1] == 0){
            options.push(enemy[index]);//asa cred ca va fi sistemul si la enemies
        }
...
function animate(currentTime) {
    ...
    i=0;
    while(i<7){
        dirt_sprites[i].update();//ups am uitat sa il pun si aici  :) lol
        i++;
    }
    ...
    //screen.drawImage(dirt, 0, 0, 180, 90);
